...
**目前版本：3.14.0** · 適用於 Calibre 6.0 及以上 · 類型：**主書庫工具列動作**
...
- 轉換預設開啟「智慧分詞後再轉換」（**Jieba**）。取消勾選則使用 **OpenCC mmseg**（按詞庫最長匹配，更快）。Jieba 有助於減少「王后」被拆錯；首次可能稍慢。
- **強制轉換（覆蓋優先）**會先把混合簡繁文字統一到簡體樞軸，再重建所選繁體；它只使用內建 OpenCC 規則，能擴大轉換範圍，但可能犧牲地區用詞精準度，並不等同於錯別字校對。此選項必須搭配雙語批註，以便在下方保留原文。雙語批註預設關閉。
- 新產生的書籍預設增加識別後綴，例如 `_繁體中文_香港_双语标注_07-29_21-34`；末尾記錄本地月、日、小時和分鐘，同一分鐘產生多本時依序增加 `_2`、`_3`。可在進階選項中關閉此後綴。
- **進階選項**分組更清晰，切換引號 / 雙語等選項時可即時看到**範例預覽**
- 可選**輕量輸出**：移除嵌入字體檔案，和/或移除書中全部圖片（含封面），得到更小、更快、偏純文字的電子書
