...
**当前版本：3.14.1** · 适用于 Calibre 6.0 及以上 · 类型：**主书库工具栏动作**
...
- 转换默认开启「智能分词后再转换」（**Jieba**）。取消勾选则使用 **OpenCC mmseg**（按词库最长匹配，更快）。Jieba 有助于减少「王后」被拆错；首次可能稍慢。
- **强制转换（覆盖优先）**会先把混合简繁文本统一到简体枢轴，再重建所选繁体；它只使用内置 OpenCC 规则，能扩大转换面积，但可能牺牲地区用词精度，并不等同于错别字校对。此选项必须配合双语批注，以便在下方保留原文。双语批注默认关闭。
- 新生成的书籍默认增加识别后缀，例如 `_繁體中文_香港_双语标注_07-29_21-34`；末尾记录本地月、日、小时和分钟，同一分钟生成多本时依次增加 `_2`、`_3`。可在高级选项中关闭此后缀。
- **高级选项**分组更清晰，切换引号 / 双语等选项时可即时看到**示例预览**
- 可选**轻量输出**：移除嵌入字体文件，和/或移除书中全部图片（含封面），得到更小、更快、偏纯文本的电子书
